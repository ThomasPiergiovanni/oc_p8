...
class ResutlView(View):
    ...
    def get(self, request):
        ...
        self.data['context']['searched_product'] = SupersubManager()._get_product(session_prod_id)
        self.data['context']["page_object"] = SupersubManager()._get_page_from_session_variables(request, session_prods_ids)
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if matching_products:
                self.data['context']['searched_product'] = matching_products[0]
                self.data['context']["page_object"] = SupersubManager()._get_page_from_form(request, matching_products[0]) 
                return render(request, self.data['render'], self.data['context'])
            else:
